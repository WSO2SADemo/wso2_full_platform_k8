...
import { aiAgent } from '../api/client';

const SUGGESTIONS = [
  'Show me all recovery plans',
  'Trigger recovery for flight FL001 due to weather delay',
  'What is the status of the latest recovery plan?',
  'How many passengers were affected by disruptions today?',
];
...
function TokenBadge({ label, token }) {
  ...
  return (
    <div className="border border-slate-200 rounded-lg p-3 bg-white">
      ...
      <details>
        <summary className="text-[10px] text-blue-600 cursor-pointer hover:text-blue-800">Show JWT</summary>
        ...
        {decoded && (
          <>
            <summary className="text-[10px] text-blue-600 cursor-pointer hover:text-blue-800 mt-1">Decoded payload</summary>
      ...
      <div className={`max-w-[75%] rounded-2xl px-4 py-3 ${
        ...
        {!isUser && (
          <div className="flex items-center gap-2 mb-1 text-xs text-slate-500">
            <span>🤖</span> <span className="font-semibold">Admin Recovery Agent</span>
            {msg.functionCalled && (
        ...
        {msg.consentUrl && (
          <button
            ...
            className="mt-2 inline-flex items-center gap-1.5 bg-blue-600 text-white text-xs font-medium rounded-lg px-3 py-2 hover:bg-blue-700 transition-colors cursor-pointer border-none"
          >
            🔐 Authorize Agent
          </button>
        )}
...
export default function AIChat() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      text: "Hello! I'm the Admin Recovery Agent powered by a local LLM. I can help you manage flight disruption recovery.\n\nTry asking me to trigger a recovery, check recovery plans, or query flight status.",
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      const resp = await aiAgent.chat(trimmed, sessionId);
      ...
      if (resp.consent_url) {
        ...
        const aiMsg = {
          role: 'assistant',
          text: resp.response || 'No response from AI agent.',
          time: new Date().toLocaleTimeString(),
      ...
      const errMsg = {
        role: 'assistant',
        text: `⚠️ Error: ${err.message}\n\nPlease try again or check if the Admin agent service is running.`,
        time: new Date().toLocaleTimeString(),
        functionCalled: null,
        functionResult: null,
      };
    ...
    <div className="flex flex-col h-[calc(100vh-2rem)]">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
            🤖 Admin Recovery Agent
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Natural language interface for disruption recovery — Powered by local LLM (Ollama)
          </p>
        </div>
      ...
      {showTokens && (
        <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <TokenBadge label="Agent Token (OBO Step 1)" token={agentToken} />
      ...
      <div className="flex gap-2">
        <textarea
          ...
          placeholder="Ask the Admin agent about flight disruptions, recovery plans..."
          rows={1}
          disabled={loading}
          className="flex-1 resize-none rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:opacity-60 placeholder:text-slate-400"
        />
        <button
          onClick={() => sendMessage()}
          disabled={loading || !input.trim()}
          className="rounded-xl bg-blue-600 text-white px-5 py-3 text-sm font-medium hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
          {loading ? (
