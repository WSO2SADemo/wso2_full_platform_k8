...
// The Customer Service AI agent authenticates itself, then obtains a delegated OBO token
// with user consent, allowing the agent to act on behalf of the user.
...
string agentCallbackUrl = "http://localhost:9097/cs/callback";
...
function acquireAgentToken() returns string|error {
    ...
    log:printInfo("CS Agent OBO agent token acquired successfully");
...
function generateOBOAuthUrl(string sessionId) returns string|error {
    ...
    // Scopes relevant to customer service operations (passenger-facing)
    ...
    log:printInfo(string `CS Agent OBO consent URL generated for session ${sessionId}`);
    return authUrl;
}
...
function exchangeOBOToken(string code, string state) returns error? {
    ...
    log:printInfo(string `CS Agent OBO token acquired for session ${pending.sessionId}, user: ${userSub ?: "unknown"}`);
}
