...
export default function OBOChatAgent({ accessToken, username }) {
  ...
  useEffect(() => {
    if (open && messages.length === 0) {
      setMessages([{
        role: 'agent',
        text: "Hello! I'm your OBO-enabled insurance assistant. For read-only queries (policies, claims) I'll respond directly. For privileged operations like submitting or updating claims I'll ask for explicit user authorization first — that's the OBO flow in action."
      }]);
    }
  }, [open]);
        ...
        <div style={{
          ...
          <div style={{
            ...
            <textarea
              ...
              onKeyDown={onKeyDown}
              placeholder="Query policies, or try: submit a claim on behalf of a customer…"
              rows={1}
