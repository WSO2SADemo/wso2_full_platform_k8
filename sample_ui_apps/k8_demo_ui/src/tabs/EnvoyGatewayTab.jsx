...
export default function EnvoyGatewayTab() {
  ...
  const realBase = import.meta.env.VITE_ENVOY_BASE || 'https://envoygw.wso2.com:8443';
  ...
  const getUrl     = () => apiPath(config.envoyBase);
  ...
  return (
    <div>
      <div className="gateway-header" style={{ background: `linear-gradient(135deg, ${COLOR}, #007ab8)` }}>
        <div style={{ fontSize: '2rem' }}>🔵</div>
        <div>
          <h2>Envoy Gateway — Patients API</h2>
          <p>Auth Code Grant via is.wso2.com · Bearer token forwarded to envoygw.wso2.com:8443</p>
        </div>
      </div>
      ...
      <ResponsePanel response={response} status={status} label="Patients API Response (Envoy Gateway)" />
