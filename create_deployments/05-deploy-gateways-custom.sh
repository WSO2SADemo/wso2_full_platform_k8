...
helm install gw "$ROOT_DIR/k8_deployments/am-gateway-custom/wso2am-universal-gw" \
  -n apim-gw \
  -f "$ROOT_DIR/k8_deployments/am-gateway-custom/gw-min.yaml"
...
if [ "$DEPLOY_EXTGW" = true ]; then
  ...
  helm install extgw "$ROOT_DIR/k8_deployments/am-gateway-custom/wso2am-universal-gw" \
    -n apim-gw \
    -f "$ROOT_DIR/k8_deployments/am-gateway-custom/extgw-min.yaml"
